...
## Status
The UI renders the board with active and ghost pieces, inputs (including hold) are wired, auto-drop ticking runs each frame, and pause/game-over overlays plus next/hold previews are shown. Soft drop acceleration is input-driven with a short grace window, and left/right movement uses DAS/ARR repeat logic. Rotation now uses SRS kick tables for I/J/L/S/T/Z pieces. Level speed follows a stepped curve and line clears briefly pause gravity with a flash effect; game over adds a red tint overlay and hides active/ghost pieces during line-clear pause. Lock delay resets are limited while grounded to prevent infinite stalling, with a pulsing lock-warning flash near expiry, a lock-delay bar in the HUD, and a landing spark highlight on lock. Combos, back-to-back bonuses, and T-spin detection/scoring (full vs mini) are included with a HUD indicator when B2B is active. Paused state ignores gameplay inputs. Sound events are emitted and, if `assets/sfx/` contains matching WAVs, played through the cpal mixer.
